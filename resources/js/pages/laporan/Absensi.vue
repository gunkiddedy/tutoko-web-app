            ...
            <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
                <main class="w-full flex-grow p-6 bg-white">
                    <div class="flex items-center justify-between border-b-2 border-gray-300">
                        <h1 class="text-lg pb-1 font-semibold text-indigo-400 uppercase">Laporan Absensi</h1>
                    </div>
                        ...
                        <table class="table-auto border-collapse border border-gray-400 w-full">
                            <thead class="text-center">
                                <tr>
                                    ...
                                    <th class="border border-gray-300 p-2">TGL KERJA</th>
                                    <th class="border border-gray-300 p-2">JAM KERJA</th>
                                    <th class="border border-gray-300 p-2">GAJI HARIAN</th>
                                    <th class="border border-gray-300 p-2">PENDAPATAN</th>
                                </tr>
                            </thead>
                            <tbody class="text-center">
                                <tr v-for="(data, index) in dataAbsensi" :key="index">
                                    <td class="border border-gray-300 p-2">{{ index+1 }}</td>
                                    <td class="border border-gray-300 p-2 text-left">{{ data.pegawai_nama }}</td>
                                    <td class="border border-gray-300 p-2 text-left">{{ moment(data.tanggal).format('L') }}</td>
                                    <td class="border border-gray-300 p-2 text-left">{{ data.jam_kerja }} Hari</td>
                                    <td class="border border-gray-300 p-2 font-bold">{{ toRupiah(data.gaji_harian) }}</td>
                                    <td class="border border-gray-300 p-2 font-bold">{{ toRupiah(data.jumlah_gaji) }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
export default {
    ...
    methods: {
        ...
        getData(){
            ...
            axios.get("/api/absensi/" + fixDate1 + '/' + fixDate2)
                .then((response) => {
                    if(response.data.length)
                        this.dataAbsensi = response.data;
                    else
                        this.dataAbsensi = 'tidak ada data';
